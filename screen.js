(function () {
  const districtGeoJson = {"type":"FeatureCollection","features":[{"type":"Feature","properties":{"adcode":330108,"name":"滨江区","center":[120.21062,30.206615],"centroid":[120.185306,30.18046],"childrenNum":0,"level":"district","acroutes":[100000,330000,330100],"parent":{"adcode":330100}},"geometry":{"type":"MultiPolygon","coordinates":[[[[120.221058,30.237666],[120.218373,30.235087],[120.213307,30.230547],[120.211297,30.228716],[120.208612,30.226514],[120.203348,30.222039],[120.196561,30.216567],[120.187584,30.211086],[120.182867,30.207929],[120.177622,30.205625],[120.165029,30.20131],[120.148694,30.195716],[120.142721,30.19364],[120.140885,30.193007],[120.140174,30.192838],[120.136944,30.19197],[120.134121,30.190587],[120.131764,30.188743],[120.129668,30.18685],[120.12804,30.184778],[120.126671,30.18283],[120.125951,30.18111],[120.125111,30.179444],[120.124626,30.177352],[120.124391,30.172199],[120.12459,30.167261],[120.125128,30.163251],[120.126001,30.159457],[120.127474,30.156009],[120.130377,30.151164],[120.133044,30.147956],[120.138594,30.142077],[120.146144,30.137174],[120.148451,30.138966],[120.151484,30.141541],[120.151779,30.142047],[120.152336,30.142541],[120.152773,30.142747],[120.153497,30.142937],[120.154209,30.143183],[120.154693,30.143925],[120.154942,30.144073],[120.155892,30.144333],[120.155305,30.145231],[120.154992,30.145199],[120.154744,30.145958],[120.155311,30.146005],[120.155667,30.146189],[120.156763,30.147137],[120.157946,30.147951],[120.158722,30.14868],[120.159077,30.148889],[120.159903,30.148223],[120.160536,30.147474],[120.160276,30.147127],[120.159646,30.147089],[120.159665,30.146401],[120.159794,30.146044],[120.160135,30.146121],[120.161411,30.146764],[120.162004,30.147104],[120.162314,30.147387],[120.162887,30.148108],[120.163804,30.148916],[120.164114,30.149848],[120.164218,30.150019],[120.164363,30.150089],[120.164815,30.150059],[120.165645,30.149592],[120.165798,30.149784],[120.166304,30.149541],[120.166459,30.149609],[120.167039,30.149247],[120.167748,30.149061],[120.168275,30.148647],[120.169313,30.149234],[120.169687,30.14953],[120.16992,30.1499],[120.170471,30.151121],[120.171031,30.151651],[120.171291,30.151832],[120.172924,30.152478],[120.173428,30.15277],[120.174529,30.153646],[120.175204,30.153887],[120.176357,30.152938],[120.1767,30.152381],[120.177282,30.151913],[120.177653,30.151412],[120.178158,30.150979],[120.178569,30.150268],[120.178862,30.149422],[120.180332,30.149818],[120.180668,30.149326],[120.18116,30.148164],[120.181388,30.147826],[120.181723,30.147243],[120.18314,30.146317],[120.184041,30.145388],[120.184139,30.145363],[120.186651,30.146597],[120.186758,30.14656],[120.187069,30.145964],[120.186185,30.144433],[120.186265,30.144335],[120.186562,30.144447],[120.187224,30.144965],[120.188097,30.145145],[120.188179,30.145304],[120.188479,30.145566],[120.188746,30.145586],[120.190779,30.146556],[120.191541,30.146825],[120.191791,30.14716],[120.19188,30.147118],[120.192554,30.145647],[120.19322,30.144793],[120.193958,30.144826],[120.195352,30.142458],[120.196023,30.143079],[120.196262,30.143133],[120.196385,30.142888],[120.196015,30.142564],[120.196031,30.142421],[120.196501,30.142238],[120.196781,30.141628],[120.196842,30.141218],[120.19699,30.140893],[120.197165,30.140764],[120.198019,30.140516],[120.199551,30.13969],[120.200471,30.139436],[120.201378,30.13956],[120.202359,30.139871],[120.202959,30.140163],[120.204085,30.141057],[120.204503,30.142364],[120.204752,30.142711],[120.205039,30.142921],[120.205262,30.143025],[120.205641,30.142611],[120.205885,30.142215],[120.207219,30.140855],[120.207884,30.139947],[120.208046,30.139301],[120.209109,30.138965],[120.209555,30.138712],[120.20952,30.138274],[120.208739,30.13756],[120.208691,30.137251],[120.208742,30.137131],[120.210281,30.136504],[120.210951,30.136342],[120.211569,30.136376],[120.213295,30.137893],[120.214533,30.136915],[120.214714,30.136913],[120.215921,30.137828],[120.217398,30.138647],[120.218973,30.139728],[120.21947,30.139948],[120.219436,30.140051],[120.218801,30.140584],[120.218286,30.141561],[120.216933,30.143075],[120.216922,30.143299],[120.21706,30.143445],[120.217618,30.143524],[120.217699,30.143706],[120.217644,30.144917],[120.21833,30.145888],[120.218789,30.147601],[120.218904,30.14778],[120.219234,30.148027],[120.220174,30.148424],[120.220334,30.148576],[120.220309,30.148731],[120.21986,30.149268],[120.219853,30.149489],[120.220016,30.150049],[120.219899,30.150514],[120.220105,30.150769],[120.220585,30.151225],[120.22057,30.151896],[120.221061,30.152526],[120.221659,30.153759],[120.223046,30.153813],[120.223807,30.153504],[120.224188,30.153495],[120.22443,30.153591],[120.224592,30.153866],[120.224576,30.154195],[120.224379,30.155129],[120.224175,30.155523],[120.223138,30.15669],[120.223169,30.157257],[120.223239,30.157441],[120.223479,30.157708],[120.225774,30.159177],[120.225853,30.159582],[120.226174,30.160155],[120.226082,30.160374],[120.225331,30.161289],[120.225099,30.161799],[120.225024,30.162066],[120.225043,30.162656],[120.224975,30.162712],[120.22377,30.162598],[120.223136,30.162777],[120.222762,30.162807],[120.2219,30.16272],[120.221269,30.162432],[120.220538,30.162581],[120.220299,30.162559],[120.220059,30.162396],[120.219817,30.162012],[120.219629,30.161868],[120.219116,30.161748],[120.218652,30.161799],[120.218535,30.161576],[120.217946,30.161303],[120.217869,30.161315],[120.217727,30.161521],[120.217591,30.161542],[120.216859,30.16132],[120.216749,30.161433],[120.216719,30.162347],[120.216568,30.162382],[120.21655,30.162712],[120.218127,30.163232],[120.218035,30.163641],[120.218222,30.163788],[120.218224,30.164162],[120.217985,30.164506],[120.217946,30.164753],[120.218469,30.165197],[120.218689,30.165493],[120.219519,30.166093],[120.21948,30.166831],[120.219966,30.167029],[120.219943,30.1672],[120.219362,30.168161],[120.219357,30.168367],[120.219582,30.168605],[120.219455,30.168706],[120.218892,30.1688],[120.218863,30.168879],[120.219003,30.16894],[120.219655,30.168925],[120.2203,30.169686],[120.220315,30.169952],[120.219936,30.169766],[120.219997,30.170025],[120.22055,30.170523],[120.220884,30.17057],[120.221182,30.170495],[120.221388,30.170606],[120.221718,30.170632],[120.22204,30.17084],[120.22223,30.171062],[120.224395,30.17125],[120.224375,30.171648],[120.224058,30.172268],[120.224013,30.172775],[120.223553,30.173695],[120.223417,30.173849],[120.223236,30.173891],[120.222285,30.173643],[120.221875,30.17442],[120.221398,30.175512],[120.222014,30.175496],[120.222031,30.176295],[120.222045,30.178883],[120.223863,30.178899],[120.224656,30.179167],[120.224682,30.179393],[120.224447,30.181476],[120.224576,30.181885],[120.233498,30.18204],[120.233677,30.183505],[120.23531,30.1831],[120.236733,30.182979],[120.236837,30.18458],[120.233982,30.185901],[120.234065,30.187528],[120.234198,30.188784],[120.234249,30.195259],[120.234317,30.201283],[120.234399,30.208242],[120.234512,30.209543],[120.23442,30.20999],[120.234459,30.213544],[120.234952,30.213663],[120.235111,30.225426],[120.235232,30.228138],[120.235041,30.229126],[120.234435,30.229798],[120.233707,30.229988],[120.231887,30.230181],[120.227398,30.232514],[120.226516,30.232896],[120.22396,30.2363],[120.221058,30.237666]]]]}}]};

  const mockData = {
    overview: {
      memberTotal: 1286,
      memberToday: 36,
      memberBound: 1108,
      memberActive: 742
    },
    exam: {
      passed: 964,
      total: 1120,
      today: 88,
      pending: 156,
      newPapers: 4
    },
    weather: {
      temp: 19,
      text: '晴转多云'
    },
    operation: {
      videoFinished: 682,
      chatCount: 412,
      onlineMembers: 137,
      activeRate: 78
    },
    streets: [
      {
        key: 'west',
        name: '西兴街道',
        count: 326,
        color: '#38bdf8'
      },
      {
        key: 'middle',
        name: '长河街道',
        count: 472,
        color: '#1d4ed8'
      },
      {
        key: 'south',
        name: '浦沿街道',
        count: 488,
        color: '#34d399'
      }
    ]
  };
  
  const streetCoordMap = {
    '西兴街道': [120.21, 30.20],
    '长河街道': [120.18, 30.18],
    '浦沿街道': [120.16, 30.15]
  };
  const geojson = window.YOUR_GEOJSON_DATA || districtGeoJson;
  let mapChart = null;
  let ambientLayerData = null;

  function formatNumber(value) {
    return String(Number(value || 0));
  }

  function formatPercent(value) {
    return Number(value || 0).toFixed(1).replace(/\.0$/, '') + '%';
  }

  function updateClock() {
    const now = new Date();
    const weekText = ['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'][now.getDay()];
    const timeText = pad(now.getHours()) + ':' + pad(now.getMinutes()) + ':' + pad(now.getSeconds());
    const dateText = now.getFullYear() + '年' + pad(now.getMonth() + 1) + '月' + pad(now.getDate()) + '日';

    setText('screenTime', timeText);
    setText('screenDate', dateText);
    setText('calendarDay', String(now.getDate()));
    setText('calendarYearMonth', now.getFullYear() + '年' + pad(now.getMonth() + 1) + '月');
    setText('calendarWeek', weekText);
  }

  function pad(value) {
    return value < 10 ? '0' + value : String(value);
  }

  function setText(id, text) {
    const el = document.getElementById(id);
    if (el) {
      el.textContent = text;
    }
  }

  function renderOverview(data) {
    setText('memberTotal', formatNumber(data.memberTotal));
    setText('memberToday', formatNumber(data.memberToday));
    setText('memberBound', formatNumber(data.memberBound));
    setText('memberActive', formatNumber(data.memberActive));
  }

  function renderExam(data) {
    const passRate = data.total ? (data.passed / data.total) * 100 : 0;
    setText('examPassed', formatNumber(data.passed));
    setText('examPassRate', formatPercent(passRate));
    setText('examToday', formatNumber(data.today));
    setText('examPending', formatNumber(data.pending));
    setText('examNewPapers', formatNumber(data.newPapers));

    const bar = document.getElementById('examPassBar');
    if (bar) {
      bar.style.width = Math.max(8, Math.min(passRate, 100)) + '%';
    }
  }

  function renderWeather(data) {
    setText('weatherTemp', formatNumber(data.temp) + '°C');
    setText('weatherDesc', data.text || '--');
  }

  function renderOperation(data) {
    setText('videoFinished', formatNumber(data.videoFinished));
    setText('chatCount', formatNumber(data.chatCount));
    setText('onlineMembers', formatNumber(data.onlineMembers));
    setText('activeRate', formatPercent(data.activeRate));
  }

  function toStreetData(streets) {
    return streets.map(function (item) {
      const coord = streetCoordMap[item.name] || [120.18, 30.18];
      return { name: item.name, value: Number(item.count || 0), coord: coord };
    });
  }

  function createSeededRandom(seed) {
    let value = seed;
    return function () {
      value = (value * 9301 + 49297) % 233280;
      return value / 233280;
    };
  }

  function getGeoBounds(sourceGeoJson) {
    const bounds = {
      minLng: Infinity,
      maxLng: -Infinity,
      minLat: Infinity,
      maxLat: -Infinity
    };
    if (!sourceGeoJson || !sourceGeoJson.features) {
      return bounds;
    }
    sourceGeoJson.features.forEach(function (feature) {
      const groups = (((feature || {}).geometry || {}).coordinates) || [];
      groups.forEach(function (polygonGroup) {
        polygonGroup.forEach(function (ring) {
          ring.forEach(function (point) {
            bounds.minLng = Math.min(bounds.minLng, point[0]);
            bounds.maxLng = Math.max(bounds.maxLng, point[0]);
            bounds.minLat = Math.min(bounds.minLat, point[1]);
            bounds.maxLat = Math.max(bounds.maxLat, point[1]);
          });
        });
      });
    });
    return bounds;
  }

  function buildAmbientLayerData(sourceGeoJson, center) {
    if (ambientLayerData) {
      return ambientLayerData;
    }

    const bounds = getGeoBounds(sourceGeoJson);
    const rand = createSeededRandom(20260327);
    const points = [];
    const lngSpan = Math.max(0.001, bounds.maxLng - bounds.minLng);
    const latSpan = Math.max(0.001, bounds.maxLat - bounds.minLat);

    for (let i = 0; i < 36; i += 1) {
      const lng = bounds.minLng + lngSpan * rand();
      const lat = bounds.minLat + latSpan * rand();
      points.push({
        name: '',
        value: [lng, lat, 2 + Math.round(rand() * 4)]
      });
    }

    points.push({
      name: '滨江中心',
      value: [center[0], center[1], 8]
    });

    ambientLayerData = {
      points: points
    };
    return ambientLayerData;
  }

  function renderDistrictMap(streets) {
    const chartEl = document.getElementById('mapChart');
    if (!chartEl || !window.echarts || !geojson) {
      return;
    }

    if (!mapChart) {
      mapChart = window.echarts.init(chartEl);
    }
    if (!window.echarts.getMap('binjiang')) {
      window.echarts.registerMap('binjiang', geojson);
    }

    const streetData = toStreetData(streets);
    const maxValue = Math.max.apply(null, streetData.map(function (item) {
      return item.value;
    }).concat([1]));
    const center = (geojson.features && geojson.features[0] && geojson.features[0].properties && geojson.features[0].properties.centroid) || [120.185, 30.18];

    const effectScatterData = streetData.map(function (item) {
      return { name: item.name, value: [item.coord[0], item.coord[1], item.value] };
    });

    const lineData = streetData.map(function (item) {
      return {
        fromName: item.name,
        toName: '滨江中心',
        coords: [item.coord, center],
        value: item.value
      };
    });
    const ambientData = buildAmbientLayerData(geojson, center);

    const option = {
      backgroundColor: 'transparent',
      animationDuration: 1400,
      animationEasing: 'cubicOut',
      tooltip: {
        trigger: 'item',
        backgroundColor: 'rgba(5, 16, 35, 0.92)',
        borderColor: '#00e5ff',
        borderWidth: 1,
        textStyle: { color: '#dff9ff', fontSize: 12 },
        formatter: function (params) {
          if (params.seriesType === 'lines') {
            return params.data.fromName + ' -> ' + params.data.toName + '<br/>流量: ' + params.data.value;
          }
          if (params.seriesType === 'effectScatter') {
            return params.name + '<br/>会员: ' + params.value[2] + ' 人';
          }
          return params.name || '滨江区';
        }
      },
      graphic: [{
        type: 'circle',
        left: 'center',
        top: 'middle',
        shape: { r: 240 },
        silent: true,
        style: {
          fill: new window.echarts.graphic.RadialGradient(0.5, 0.5, 1, [
            { offset: 0, color: 'rgba(0,255,255,0.10)' },
            { offset: 1, color: 'rgba(0,50,120,0)' }
          ])
        }
      }, {
        type: 'circle',
        left: 'center',
        top: 'middle',
        shape: { r: 184 },
        silent: true,
        style: {
          stroke: 'rgba(0,255,255,0.25)',
          fill: 'rgba(0,0,0,0)',
          lineWidth: 1.2,
          shadowBlur: 18,
          shadowColor: 'rgba(0,255,255,0.45)'
        }
      }, {
        type: 'circle',
        left: 'center',
        top: 'middle',
        shape: { r: 136 },
        silent: true,
        style: {
          stroke: 'rgba(125,249,255,0.22)',
          fill: 'rgba(0,0,0,0)',
          lineWidth: 1,
          shadowBlur: 16,
          shadowColor: 'rgba(125,249,255,0.42)'
        }
      }],
      geo: {
        map: 'binjiang',
        roam: false,
        zoom: 0.96,
        itemStyle: {
          areaColor: new window.echarts.graphic.LinearGradient(0, 0, 0, 1, [
            { offset: 0, color: '#0a2b66' },
            { offset: 0.5, color: '#0b3f88' },
            { offset: 1, color: '#071c3a' }
          ]),
          borderColor: '#00ffff',
          borderWidth: 1.6,
          shadowColor: 'rgba(0,255,255,0.55)',
          shadowBlur: 18
        },
        emphasis: {
          itemStyle: {
            areaColor: '#135fbd',
            borderColor: '#66ffff',
            borderWidth: 2.4,
            shadowColor: 'rgba(102,255,255,0.9)',
            shadowBlur: 30
          },
          label: { show: false }
        }
      },
      series: [
        {
          name: '底图',
          type: 'map',
          map: 'binjiang',
          geoIndex: 0,
          silent: true,
          itemStyle: { opacity: 0 }
        },
        {
          name: '数据流',
          type: 'lines',
          coordinateSystem: 'geo',
          zlevel: 2,
          effect: {
            show: true,
            period: 4,
            trailLength: 0.45,
            symbol: 'circle',
            symbolSize: 5,
            color: '#7df9ff'
          },
          lineStyle: {
            color: '#19d4ff',
            width: 1.2,
            opacity: 0.38,
            curveness: 0.2
          },
          data: lineData
        },
        {
          name: '背景粒子',
          type: 'scatter',
          coordinateSystem: 'geo',
          zlevel: 2,
          silent: true,
          symbolSize: function (val) {
            return val[2];
          },
          itemStyle: {
            color: 'rgba(125,249,255,0.55)',
            shadowBlur: 8,
            shadowColor: 'rgba(125,249,255,0.55)'
          },
          data: ambientData.points
        },
        {
          name: '街道会员',
          type: 'effectScatter',
          coordinateSystem: 'geo',
          zlevel: 3,
          rippleEffect: { scale: 4.2, brushType: 'stroke' },
          showEffectOn: 'render',
          label: {
            show: true,
            color: '#d9fcff',
            fontSize: 12,
            offset: [0, -18],
            formatter: function (params) {
              return params.name + '\n' + params.value[2] + ' 人';
            }
          },
          symbolSize: function (val) {
            return 10 + (val[2] / maxValue) * 16;
          },
          itemStyle: {
            color: '#00f6ff',
            shadowBlur: 26,
            shadowColor: 'rgba(0,246,255,0.9)'
          },
          emphasis: {
            scale: true,
            itemStyle: {
              color: '#b2ffff',
              shadowBlur: 34,
              shadowColor: 'rgba(178,255,255,1)'
            }
          },
          data: effectScatterData
        },
        {
          name: '中心能量',
          type: 'effectScatter',
          coordinateSystem: 'geo',
          zlevel: 4,
          silent: true,
          rippleEffect: { scale: 7, brushType: 'stroke' },
          symbolSize: 10,
          itemStyle: {
            color: '#7df9ff',
            shadowBlur: 24,
            shadowColor: 'rgba(125,249,255,1)'
          },
          data: [{
            name: '滨江中心',
            value: [center[0], center[1], 1]
          }]
        }
      ]
    };

    mapChart.setOption(option, true);
  }

  function renderStreets(streets) {
    const total = streets.reduce(function (sum, item) {
      return sum + Number(item.count || 0);
    }, 0);

    setText('donutTotal', formatNumber(total));

    const ranked = streets.slice().sort(function (a, b) {
      return Number(b.count || 0) - Number(a.count || 0);
    });

    const rankList = document.getElementById('streetRankList');
    if (rankList) {
      const top = ranked[0] ? Number(ranked[0].count || 0) : 1;
      rankList.innerHTML = ranked.map(function (item, index) {
        const width = top ? Math.max(10, (Number(item.count || 0) / top) * 100) : 0;
        return ''
          + '<div class="rank-item">'
          + '  <div class="rank-order">' + (index + 1) + '</div>'
          + '  <div>'
          + '    <div class="rank-name">' + escapeHtml(item.name) + '</div>'
          + '    <div class="rank-bar"><div class="rank-bar-fill" style="width:' + width + '%"></div></div>'
          + '  </div>'
          + '  <div class="rank-count">' + formatNumber(item.count) + '</div>'
          + '</div>';
      }).join('');
    }

    const donut = document.getElementById('streetDonut');
    if (donut) {
      let start = 0;
      const parts = ranked.map(function (item) {
        const ratio = total ? (Number(item.count || 0) / total) * 100 : 0;
        const end = start + ratio;
        const segment = item.color + ' ' + start.toFixed(2) + '% ' + end.toFixed(2) + '%';
        start = end;
        return segment;
      });
      donut.style.background = 'conic-gradient(' + parts.join(', ') + ')';
    }

    const legend = document.getElementById('streetLegend');
    if (legend) {
      legend.innerHTML = ranked.map(function (item) {
        const ratio = total ? (Number(item.count || 0) / total) * 100 : 0;
        return ''
          + '<div class="legend-item">'
          + '  <div class="legend-left">'
          + '    <span class="legend-dot" style="background:' + item.color + '"></span>'
          + '    <span class="legend-name">' + escapeHtml(item.name) + '</span>'
          + '  </div>'
          + '  <div class="legend-percent">' + formatPercent(ratio) + '</div>'
          + '</div>';
      }).join('');
    }

    renderDistrictMap(streets);
  }

  function escapeHtml(value) {
    return String(value == null ? '' : value)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
      .replace(/"/g, '&quot;')
      .replace(/'/g, '&#39;');
  }

  function bootstrap() {
    renderOverview(mockData.overview);
    renderExam(mockData.exam);
    renderWeather(mockData.weather);
    renderOperation(mockData.operation);
    renderStreets(mockData.streets);
    updateClock();
    window.setInterval(updateClock, 1000);
    window.addEventListener('resize', function () {
      if (mapChart) {
        mapChart.resize();
      }
    });
  }

  bootstrap();
})();
